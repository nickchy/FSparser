...
#my $file=$ARGV[0];
my $file="F:\\Works\\Research_database\\jultop1000v.txt";
...
$output="F:\\Works\\Research_database\\jultop1000voutput.txt";
...
print "$factorkey \n";
   foreach my $datekey (sort { $a <=> $b } keys %{$dataset{$factorkey}}) {
     foreach $quartilekey(sort { $a <=> $b } keys %{$dataset{$factorkey}{$datekey}})
{
